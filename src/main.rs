...
async fn main(){
    let user = User {
        name: "Abhishek Swami".to_string(),
        age: 21,
        email:"abhishekgamer959@gmail.com".to_string(),   
    };

    let firebase = Firebase::new("https://rust-project-a22eb-default-rtdb.firebaseio.com/").unwrap();
...
async fn get_user(firebase_client: &Firebase) -> HashMap<String, User>{
    let firebase = firebase_client.at("users");
...
async fn delete_user(firebase_client: &Firebase, id: &String){
    let firebase = firebase_client.at("users").at(&id);
    let result = firebase = firebase.delete().await;
}
